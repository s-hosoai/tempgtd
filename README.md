...
- Node.js（推奨: v20以上。nvm等で管理している場合は`nvm use`で切り替え）
- pnpm（未インストールの場合は下記の通りcorepackで有効化）

### セットアップ

```bash
# pnpmが未インストールならNode付属のcorepackで有効化
corepack enable
...
`.env`ファイルは不要。以下の挙動により、追加設定なしでそのまま開発できる。

- `AUTH_PASSWORD`を設定しない場合、認証なしで起動する（開発用の抜け道）
- `DB_PATH`を設定しない場合、DBはプロジェクトルートの`./gtd.db`に作成される
- DBのテーブル作成・マイグレーションはアプリ起動後の初回アクセス時に自動実行される（`lib/db/index.ts`）。手動でのマイグレーションコマンド実行は不要

本番相当の設定（認証あり・別のDB_PATH等）で動かしたい場合は、[環境変数ファイル作成](#5-環境変数ファイル作成)を参考に`.env`を作成する。
